...
if (isset($_SESSION['booking_price'])) {
    // Retrieve and display the price
    $price = $_SESSION['booking_price'];
     // You can use this variable wherever you want to display the price
}
$sql="select *from booking where email='$email' order by book_id DESC "; //jbna m3lumet tb3 user yali 3amel login
$cname = mysqli_query($con,$sql); //nfzna query($con tb3 connection)
$email = mysqli_fetch_assoc($cname); //bi7otole m3lumet tb3 row yali tl3 m3e bil query fo2 bi array email
$bid=$email['book_id']; //a5dna id tb3o 3shen bs bdna nd5el payment 3al table bdna nd5el book id tb3a
$_SESSION['bid']=$bid;
...
if(isset($_POST['pay'])){
  ...
  if(empty($cardno) || empty($exp) ||  empty($cvv)  ){
    echo '<script>alert("please fill the place")</script>'; //bde 8ayyer alert.
  }
  else{
    $sql2="insert into payment (book_id,CARD_NO,EXP_DATE,CVV,PRICE) values($bid,'$cardno','$exp',$cvv,$price)"; //zedna payment if successful
...
//note:lezem tkun esem tb3 credit card mtel esem user. bde a3melaaa
...
<html lang="en">
  <head>
    ...
    <script type="text/javascript">
        function preventBack() { //hyde function ma bt5line erja3 min page y3ne ekbos sahem yali fo2 w edhar lezem 5ales w ekbs aw cancel aw pay
            window.history.forward(); //ma b5line erja3
        }
          
        setTimeout("preventBack()", 0); //to run the function immediatelly
          
        window.onunload = function () { null }; //3shen bs ekbs ino bde odhar ma yma7ile kel shi 3shen hek null
    </script>
  </head>
<body>
<style>
...
.fields-container .field-container:last-child {
  ...
  <div class="payment-title">
	...
	<form method="POST">
        ...
        <div class="field-container"><br>
            <label for="cardnumber">Card Number</label><span id="generatecard">generate random</span><br>
            <input id="cardnumber" name="cardno" type="text" pattern="[0-9]{4}\s[0-9]{4}\s[0-9]{4}\s[0-9]{4}" inputmode="numeric">
    ...
    <div class="field-group">
        <div class="field-container">
            ...
            <input id="expirationdate" name="exp" type="text" pattern="\d{2}/\d{2}" inputmode="numeric">
        </div>
        <div class="field-container">
            <label for="securitycode">Security Code</label>
            <input id="securitycode" name="cvv" type="text" pattern="\d{4}" inputmode="numeric">
        </div></div></div>
