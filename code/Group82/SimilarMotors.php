...
$NotAvailable=$_POST['motorID']; 
$query = "SELECT * FROM motorcycle
WHERE ID <> $NotAvailable
AND ( Brand = (SELECT Brand FROM motorcycle WHERE ID = $NotAvailable)
OR engine = (SELECT engine FROM motorcycle WHERE ID = $NotAvailable)
OR fuel = (SELECT fuel FROM motorcycle WHERE ID = $NotAvailable))
AND Available='Yes'" ;
...
if ($res->num_rows > 0) {
    ...
    while ($data = mysqli_fetch_array($res)) {
        ...
        echo '<img src="' . $data['image'] . '" style="height:50%;max-width: 100%; border-radius: 8px; margin-bottom: 10px;"';
