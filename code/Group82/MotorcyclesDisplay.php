...
<form method="post" action="search.php">
    ...
    <button type="submit" name="Search" style="width:7%;padding: 10px; background-color: rgb(179, 44, 48); color: #fff; border: none; border-radius: 4px; cursor: pointer; transition: background-color 0.3s ease, color 0.3s ease;" onmouseover="this.style.backgroundColor='rgba(150, 150, 150, 0.8)'" onmouseout="this.style.backgroundColor='rgb(179, 44, 48)'">Search</button>
</form>
 
<!-- Filter Form -->
<form method="post" action="MotorcyclesDisplay.php" style="margin-top: 20px; text-align: center;">
    <label for="price_range" style="font-size: 20px; color: #fff; margin-right: 10px;">Filter by Price Range:</label><br>
    <select id="price_range" name="price_range" style="padding: 8px; font-size: 14px;">
    ...
    <button type="submit" name="Filter" style="width:7%;padding: 10px; background-color: rgb(179, 44, 48); color: #fff; border: none; border-radius: 4px; cursor: pointer; transition: background-color 0.3s ease, color 0.3s ease;" onmouseover="this.style.backgroundColor='rgba(150, 150, 150, 0.8)'" onmouseout="this.style.backgroundColor='rgb(179, 44, 48)'">Filter</button>
</form>
...
if (isset($_POST['Filter'])) {
    ...
    // Modify your SQL query to include the price range filter
    if ($price_range) {
        $query = "SELECT * FROM motorcycle WHERE price >= $price_range";
    } else {
        // Handle case when no price range is selected
        $query = "SELECT * FROM motorcycle";
    }
} else {
    // Default query to fetch all motorcycles
    $query = "SELECT * FROM motorcycle";
